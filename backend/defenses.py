...
def llm_based_detection(data, gemini_model):
    """
    LLM-Based Detection: Asks a separate instance/call of Gemini to evaluate if the input is malicious.
    ...
    try:
        ...
        if "yes" in text:
            return True, f"LLM-based detector classified this input as compromised (model output: '{response.text.strip()}')."
        return False, f"LLM-based detector classified this input as safe (model output: '{response.text.strip()}')."
    except Exception as e:
        return False, f"LLM-based detection failed due to error: {str(e)}"


def known_answer_detection(data, secret_key, gemini_model):
    """
    ...
    try:
        ...
        if secret_key.lower() not in text:
            return True, f"Known-answer check failed. The secret key '{secret_key}' was not found in the output. Model output: '{response.text.strip()}'."
        return False, f"Known-answer check passed. The secret key '{secret_key}' was found in the output. Model output: '{response.text.strip()}'."
    except Exception as e:
